...
void probThree() {
    ...
    // we cannot dereference a null pointer -> *(0x000) not possible
    // fix -> ptr = &a
    *ptr = a;
    cout << *ptr << endl;
}
...
void probFour() {
    ...
    int b = 10;
    int *a = &b;
    cout << a << " " << &b << endl;              // &b and a will have the same memory address.
}
...
void probSix() {
    int a = 7;
    int *c = &a;
    // the address of "a" var will be added with (1 * 4) byte cz of int which will give some
    // random garbage value at its next location
...
void probEleven() {
    ...
    //   // we cannot increment array which is constant stored in stack memory
    //   cout << *p << endl;
}
...
void changeSign(int *p) {
    // this will modify to value stored in p
    // use address of pointer p and the result will be stored again in the same
    *p = (*p) * -1;
}
